...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        UILabel *nameLabel = [[UILabel alloc] initWithFrame:CGRectMake(kMainScreenWidth/2-50, topLineView.bottom+interval, 100, 15)];
        ...
        UILabel *timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(25, nameLabel.bottom+interval, 130, 15)];
        timeLabel.font = kFont12;
        timeLabel.textColor = [UIColor lightGrayColor];
        timeLabel.backgroundColor = [UIColor clearColor];
        timeLabel.text = @"发布时间 : 2014-10-24";
        [self addSubview:timeLabel];
        
        UILabel *personLabel = [[UILabel alloc] initWithFrame:CGRectMake(kMainScreenWidth-90, timeLabel.top, 90, 15)];
        personLabel.font = kFont12;
        personLabel.textColor = [UIColor lightGrayColor];
        personLabel.backgroundColor = [UIColor clearColor];
        personLabel.text = @"浏览量 : 123";
        ...
        NSArray *valueArray = @[@"403",@"现货",@"欧式",@"化纤",@"窗帘",@"提花"];
        CGFloat endY = 0.0;
        for (int i=0; i<7; i++)
        {
        ...
        for (int i=0; i<6; i++)
        {
            ...
            if (i==0)
            {
                valueLabel.text = [NSString stringWithFormat:@"%@ 元/米", [valueArray objectAtIndex:i]];
            }
            else
            {
        ...
        UITextView *detailTextView = [[UITextView alloc]
